...
atomic_restore_config() {
  local src="$1"
  local tmp="$CONFIG.vps-tier-restore.$$.tmp"
...
apply_client() {
  validate_common
  validate_apply_source
  for cmd in awk awg awg-quick cp cut date grep install ip mktemp mv sha256sum stat systemctl tr; do
  ...
  printf '\n[Peer]\nPublicKey = %s\nAllowedIPs = %s\n' "$CLIENT_PUBLIC" "$CLIENT_ADDR" >> "$NEW_CONFIG"
  ...
  PROFILE_TMP="$TMP_DIR/$DEVICE.conf"
  printf '%s\n' \
    '[Interface]' \
    ...
    'PersistentKeepalive = 25' > "$PROFILE_TMP"
  unset CLIENT_PRIVATE SERVER_PUBLIC Jc Jmin Jmax S1 S2 H1 H2 H3 H4
  chmod 0600 "$PROFILE_TMP"
  awg-quick strip "$PROFILE_TMP" >/dev/null

  CONFIG_TMP="$CONFIG.vps-tier-$DEVICE.$$.tmp"
  install -o root -g root -m 0600 "$NEW_CONFIG" "$CONFIG_TMP"
  ...
  install -o root -g root -m 0600 "$PROFILE_TMP" "$PROFILE_FILE"
...
rollback_client() {
  validate_common
  for cmd in awk awg awg-quick cut grep install ip mv sha256sum systemctl; do
    command -v "$cmd" >/dev/null 2>&1 || fail "missing command: $cmd"
  done
  OUTPUT_DIR="$OUTPUT_ROOT/$DEVICE"
  STATE_FILE="$OUTPUT_DIR/state.env"
...
case "$MODE" in
  ...
  *) fail "usage: manage_awg_client.sh {apply|rollback}" ;;
esac
