...
for cmd in awk awg awg-quick grep install ip mktemp sha256sum stat systemctl tr; do command -v "$cmd" >/dev/null 2>&1 || fail "missing command: $cmd"; done
...
(( Jc >= 1 && Jc <= 128 )) || fail "Jc out of range"
(( Jmin >= 0 && Jmax >= Jmin )) || fail "Jmin/Jmax invalid"
(( S1 >= 0 && S2 >= 0 && S1 + 56 != S2 )) || fail "S1/S2 invalid"
